...
class GraphicsFrame(wx.Frame):
    ...
    def draw(self):
        ...
        self.lines = []
        self.title = "& ".join([x.lstrip(' ') for x in self.legends])
...
class CalculatorMenu:
    def __init__(self, panel):
        ...
        self.edit_menu.Append(wx.ID_FILE2, "&Degrees\tCtrl+D", "Degrees")
        ...
        self.edit_menu.Append(wx.ID_FILE5, "&Centre Graphs\tCtrl+G", "Enable centred graphs", wx.ITEM_CHECK)
        self.edit_menu.Append(wx.ID_FILE6, "&Grid Outline\tCtrl+I", "Enable grid", wx.ITEM_CHECK)
        self.edit_menu.Append(wx.ID_FILE7, "&Hi-Res\tCtrl+H", "Enable Hi-Res Mode", wx.ITEM_CHECK)
        self.edit_menu.Append(wx.ID_FILE8, "&Legend\tCtrl+J", "Enable Legend", wx.ITEM_CHECK)
        self.edit_menu.Append(wx.ID_FILE9, "&Fractions\tCtrl+F", "Enable Fractions", wx.ITEM_CHECK)
        ...
        self.edit_menu.Check(wx.ID_FILE5, UIConstants.gCentre)
        self.edit_menu.Check(wx.ID_FILE6, True)
        self.edit_menu.Check(wx.ID_FILE7, False)
        self.edit_menu.Check(wx.ID_FILE8, True)
        self.edit_menu.Check(wx.ID_FILE9, UIConstants.gFraction)
        ...
        panel.Bind(wx.EVT_MENU, self.toggleCentre, id=wx.ID_FILE5)
    ...
    def toggleCentre(self, event):
        ...
        for graph in allGraphs:
            if UIConstants.gCentre:
                l_min = min(graph[0].x[0], graph[0].y[0])
                l_max = max(graph[0].x[1], graph[0].y[1])
                graph[0].originalX = (l_min, l_max)
                graph[0].originalY = (l_min, l_max)
            else:
                graph[0].originalX = graph[0].x
                graph[0].originalY = graph[0].y
            graph[0].draw()

        self.edit_menu.Check(wx.ID_FILE5, self.edit_menu.IsChecked(wx.ID_FILE5))
        ...
        if textEntryDialog.ShowModal() == wx.ID_OK:
            ...
            if "name" in value.lower() and graphOfChoice is not None:
                ...
                if textEntryDialogN.ShowModal() == wx.ID_OK:
                    graphOfChoice[0].title = textEntryDialogN.GetValue()
                    graphOfChoice[0].draw()
